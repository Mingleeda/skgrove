// 통합 로컬 프록시 — notify(슬랙) + ai(OpenRouter)를 한 포트에서 경로로 분기한다.
// 실행: node scripts/proxy.mjs  (또는 npm run proxy)
//   POST /api/ai      → AI 취합 (ai-proxy)
//   POST /api/notify  → 슬랙 전송 (notify-proxy)
// 설정은 각각 .env.ai.local / .env.notify.local 에서 읽는다(없으면 해당 기능만 휴면).
...
createServer((req, res) => {
  ...
  console.log(`🔗 proxy (notify+ai) 실행 중 → http://127.0.0.1:${PORT}`);
